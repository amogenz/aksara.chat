...
let isSoundOn = true;
let sendOnEnter = true; // Default: Enter kirim pesan
let replyingTo = null;
let onlineUsers = {};
...
window.onload = function() {
    ...
    if(localStorage.getItem('aksara_enter')) {
        ...
        // Default true jika belum pernah di-set
...
// --- LOGIKA TOMBOL ENTER BARU ---
function handleEnter(e) {
    // Jika tekan Enter (tanpa Shift)
    if (e.key === 'Enter' && !e.shiftKey) {
        if (sendOnEnter) {
            // Jika opsi "Enter untuk Kirim" AKTIF
            e.preventDefault(); // Cegah baris baru
            sendMessage();
        }
        // Jika opsi MATI, biarkan dia membuat baris baru secara default
    }
}
